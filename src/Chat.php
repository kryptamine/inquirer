...
class Chat
{
    ...
    public function addButler()
    {
        $butler = new \stdClass();
        $butler->type = "butler";
        $butler->message = "<b>Вас приветствует Plesk Quiz Bot!</b>\n\nПрохождение квизa займет у вас примерно 15-20 минут и время ответа на каждый вопрос имеет значение при подсчете баллов.\n\nВы можете проходить каждый квиз по несколько раз, однако учитывается лишь результат <b>первой попытки</b>. Желаем удачи! \xF0\x9F\x98\x8A\n\nЧтобы начать, пожалуйста, укажите свой email (тот же, что при <b>регистрации на трансляцию</b>).";
        $butler->current = true;
        $data = $this->storage->get();
